...
subtitle: 'Lecture notes for session 3: Spatial Ecology'
---

![](images/wallace.png){width="200px" style="float:right; padding-left:10px"}
...
![](images/wallacea.png){width="300px" style="float:left; padding-right:10px"}
...
<center>
![](images/sar-west-indies.png){width="500px"}
...
![](images/wilson-macarthur.png){width="200px" style="float:right; padding-left:10px"}
...
<center>
![](images/theory-island-bio.png){width="500px"}
</center>
...
<center>
![](images/butterfly-metapop.png){width="500px"}
</center>
...
The metapopulation model we explored above is only concerned with the ***occupancy*** of the patches (or local populations) in the metapopulation, i.e. whether the species is present or not. It ignores the population dynamics playing out within each patch, which, as we know, can have profound impacts on the ability of the species to persist at local scales. Thus influencing its ability to colonise habitable patches at regional scales.

In this section we introduce population models of multiple (2) patches connected via dispersal. As we saw during the lecture, ***dispersal*** is a fundamental spatial process that plays a key role in the dynamics of metapopulations.

The aim of this section is thus to develop a dynamical model of two patches connected by dispersal, where each patch contains a local population with its own temporal dynamics affected by the same growth processes we have explored in previous sessions and that are additionally subjected to ***emigration*** and ***immigration*** processes.

<center>
![](images/2-patches-connected.png){width="300px"}
</center>

We start the modelling exercise following a discrete time approach and later explore the model’s behaviour when environmental heterogeneity is added.

The model we will implement, which considers population dynamics at the local scale, plus dispersal, is given by the following set of difference equations:

$$N_{1,t+1} = N_{1,t} f(N_{1,t} ) + m N_{2,t} f(N_{2,t}) - m N_{1,t} f(N_{1,t})$$


$$N_{2,t+1} = N_{2,t} f(N_{2,t}) + m N_{1,t} f(N_{1,t}) - m N_{2,t} f(N_{2,t})$$
where $N_{1,t}$ is the size of population 1 at time t, and f is the growth function of our choice to model population dynamics. For consistency with previous sessions we will keep $f$ as the Ricker growth function. Lastly, $m$ is the migration or dispersal rate, which determines the fraction of the population 1 that moves to population 2 and viceversa. Note that for simplicity, in this model we are keeping $m$ as a single value equal across local populations and for emigration and immigration. In many real scenarios the dispersal rate might vary across populations and the immigration rate might not need be equal to emigration.

Also, for simplicity, we are assuming full connectivity between patches (i.e. there is a pathway running from patch 1 to patch 2 and viceversa). More generally, however, and when modelling larger metapopulations (i.e. more local populations in the network) the connectivity between patches can be defined by a complex networks of distinct dispersal corridors between patches.

```{r, 2-patch-metapop, fig.align='center', message=FALSE, warning=FALSE, eval=TRUE}
# Implementation of a 2-patch discrete time model
tspan = 100
r = 2.1	#intrinsic growth rate
K = 20
m = 0 # dispersal rate
extinction = 0.01 # extinction threshold
x1 = matrix(0,1, tspan) # memory preallocation
x2 = matrix(0,1, tspan) # memory preallocation
x1[1] = runif(1); # initial population size, patch 1
x2[1] = runif(1); # initial population size, patch 2

for(t in 1:(tspan-1)){
  # density dependent growth
  x1[t+1] = x1[t]*exp(r*(1-(x1[t]/K)))
  x2[t+1] = x2[t]*exp(r*(1-(x2[t]/K)))
  
  # dispersal
  x1disp = m*x1[t+1]  #individuals leaving patch 1
...
par(mfrow=c(1,2))
plot(1:tspan, x1,type='l',col='red', xlab='Time', ylab='Abundance', main = 'Patch 1')
plot(1:tspan, x2,type='l',col='blue', xlab='Time', ylab='Abundance', main = 'Patch 2')
```


### Environmental heterogeneity

Spatial heterogeneity in environmental conditions can highly influence metapopulation dynamics. It constitutes a fundamental mechanism for species sorting and plays a key role in the emergence of source-sink dynamics.

To explore the effects of spatial heterogeneity on metapopulation dynamics we will extend the code above by adding an environmental quality term.

Insert the following lines of code in the appropriate places of the code above to incorporate these effects:

```{r, 2-patch-env, fig.align='center', message=FALSE, warning=FALSE, eval=TRUE}
# including environmental heterogeneity into the model
env1 <- .1 #environmental conditions in patch 1
env2 <- 1 #environmental conditions in patch 2

x1[t+1] = x1[t]*exp(r*(1-(x1[t]/K)))*env1
x2[t+1] = x2[t]*exp(r*(1-(x2[t]/K)))*env2
```
...
<center>
![](images/metacom-paradigms.png){width="400px"}
</center>
<center>
